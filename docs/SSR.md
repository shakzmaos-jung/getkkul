...
**REQ-D2** 요약은 사용자가 선택한 길이 모드를 따른다.
- AC-D2.1 사용자 설정의 길이 모드에 따라 아래 형식으로 생성된다.
  - 짧게: 헤드라인 1줄 + 핵심 내용 1~3문장 + 상세 불릿 2~5개
  - 보통(기본): 헤드라인 1줄 + 핵심 내용 1~7문장 + 상세 불릿 2~10개
  - 길게: 헤드라인 1줄 + 핵심 내용 1~12문장 + 상세 불릿 10~20개
- AC-D2.2 생성된 요약이 선택 모드의 형식 조건(불릿 개수 등)을 만족하는지 검증 가능하다.
- AC-D2.3 동일 영상·동일 길이 모드·동일 언어의 요약은 1회만 생성·캐시되어 재사용된다(unique: video_id + length_mode + language). 서로 다른 사용자가 같은 채널을 구독해도 (모드,언어) 조합별로 1회만 연산한다.
...
- AC-E2.3 담을 새 영상이 없으면 "새 소식 없음" 메시지를 발송한다(항상 발송).
...
**확정됨**
- 요약 길이 정의: 짧게(핵심 1~3문장 + 불릿 2~5) / 보통(1~7문장 + 불릿 2~10) / 길게(1~12문장 + 불릿 10~20). 기본 보통.
- RSS 폴링 주기: 30분.
