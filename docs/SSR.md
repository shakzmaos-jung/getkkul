...
**REQ-D2** 요약은 사용자가 선택한 길이 모드를 **정보 계층(깊이)**으로 따른다(요약품질 부록 REQ-A, ADR-0013 — 문장 수 규격 폐지).
- AC-D2.1 각 모드는 "몇 문장"이 아니라 정보 층위로 생성된다.
  - 짧게: TL;DR(무엇을 다뤘나) 핵심 한두 요점.
  - 보통(기본): 핵심 요점 + 왜 중요한지 맥락.
  - 길게: **2단락 구분** — ① 핵심 사실 요약(사실·수치·사례) ② 맥락·시사점·인사이트. 핵심 문장은 하이라이트 마킹. `body` jsonb 에 {facts, insights} 구조로 저장.
...
- **summaries**(id PK, video_id→videos, length_mode enum['short','normal','long'], language enum['ko','en'] default 'ko', headline, core_text, body jsonb{facts,insights,notProvided}, prompt_version, created_at, UNIQUE(video_id, length_mode, language))
...
**확정됨**
- 요약 길이 정의(정보 계층, ADR-0013): 짧게=TL;DR / 보통=핵심+맥락 / 길게=2단락(핵심 사실 + 맥락·인사이트, 핵심문장 하이라이트). 단조성 보장, 빈약 콘텐츠는 적응형으로 상위 모드 미제공. 기본 보통.
- RSS 폴링 주기: 30분.
