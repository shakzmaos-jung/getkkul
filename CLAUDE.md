...
- 사용자(나)는 상류 결정만 담당한다: 스펙 승인, 구현 계획 승인.
- 그 외 구현 디테일은 검증 게이트(테스트·린트·타입·CI) 통과를 조건으로 자율 진행한다. **게이트가 모두 통과하면 에이전트가 PR 머지·프로덕션 배포까지 완료한다(자기 PR 머지 포함). 계획 승인이 곧 머지 승인이다.** 게이트 실패·에스컬레이션 항목·계획에 없던 파괴적 변경은 예외(정지·확인).
