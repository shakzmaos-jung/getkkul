...
export const PROMPT_VERSION = 'sq-2026-07-18.1';
...
export function allModesSystemPrompt(language: SummaryLanguage, hint?: DomainHint): string {
  ...
  return [
    ...
    '적응형 깊이(depthCeiling): 제공할 최상위 상세도를 콘텐츠의 실질 정보량으로 판정한다 — 말투나 영상 길이가 아니라 "시청자가 새로 알게 되는 서로 다른 사실·주장·수치·사례·논거가 얼마나 많은가"로 본다. long = 그런 실질 정보가 풍부(분석·인터뷰·강연·해설·토론·뉴스 등은 대화체·독백이어도, 길든 짧든 담긴 정보가 많으면 long). normal = 실질 정보가 보통. short = 실제로 뽑아낼 정보가 적을 때만(스몰토크·반복·인사·홍보·잡담·필러 위주이거나 아주 짧은 클립 — 길어도 알맹이가 없으면 short).',
    '주의: 대화체·캐주얼·독백·잡담 섞인 말투라는 이유만으로 depthCeiling 을 낮추지 마라. 전달 형식이 아니라 담긴 실질 정보량으로 판단하라. depthCeiling 위의 모드는 억지로 부풀리지 말고 핵심만 간단히 두라(사용자에게 제공하지 않는다).',
