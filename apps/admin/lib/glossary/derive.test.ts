...
describe('glossary derive', () => {
  it('parseGlossaryQuery: 기본값 · source 화이트리스트 · q trim · page→offset', () => {
    ...
    expect(parseGlossaryQuery({ source: 'admin', q: '  NPU  ', page: '2' }, 50)).toEqual({
      source: 'admin',
      search: 'NPU',
      page: 2,
      limit: 50,
  ...
  it('totalPages · glossaryQueryString', () => {
    ...
    expect(glossaryQueryString({ source: 'llm', search: 'x', page: 2 })).toBe('?source=llm&q=x&page=2');
  });
});
