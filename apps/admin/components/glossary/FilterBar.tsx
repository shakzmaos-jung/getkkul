...
/** 용어사전 필터/검색 바. source select 즉시 적용, 검색어는 제출 시. URL searchParams 가 진실원. */
export function FilterBar({ source, search }: { source: string; search: string }) {
  const router = useRouter();
  const [q, setQ] = useState(search);

  function go(nextSource: string, nextSearch: string) {
    router.push(
