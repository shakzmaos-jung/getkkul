...
### Fixed
- **다이제스트 발송 오탐("새 소식 없음") 수정**: 준비된 새 영상이 있는데도 일부 슬롯에서
  "새 소식 없음"이 발송되던 문제(대용량 구독 사용자에서 간헐 발생). 원인은 발송 대상 선별
  (`candidateVideos`)이 사용자 채널의 `done` 영상 **전량**을 무제한 조회한 뒤 클라이언트에서
  필터하면서, 이 비대 조회가 부하 시 간헐 실패할 때 **error 를 확인하지 않고 빈 목록으로 처리**
  (`videos ?? []`)한 것. 선별을 서버측 RPC `get_deliverable_videos`(요약조인·멤버십 floor·미발송
  필터 + 상한 60)로 이관하고, 조회 실패를 조용한 빈 결과가 아니라 오류로 처리(다음 슬롯 재시도)해
  근절. 정상 동작하는 피드(`get_feed_digests`) 패턴과 일관.
