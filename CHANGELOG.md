...
### Changed
- **요약 깊이 판정 개선(긴 콘텐츠가 '간단히'만 나오던 문제)**: 요약의 depthCeiling(제공 상세도) 판정을 영상 길이·말투가 아니라 콘텐츠의 실질 정보량 기준으로 하도록 프롬프트를 재작성했다. 대화체·독백·잡담 섞인 말투라는 이유로 낮게 판정하던 것을 막아, 긴 분석·인터뷰·강연이 '자세히/최대한'까지 제공된다(진짜 알맹이 없는 영상은 여전히 '간단히'). 신규로 처리되는 영상부터 반영. (`lib/summary/summarize.ts`, `PROMPT_VERSION sq-2026-07-18.1`)

### Added
- **요약 깊이 판정 관측성**: 모델의 원본 depthCeiling 판정을 요약 `body.modelCeiling` 에 기록하고 short/normal 판정 시 경보 로그를 남겨, 오판(모델이 short 판정 vs 단조성 방어 붕괴)을 사후 구분·추적할 수 있게 했다. (`lib/summary/get-or-create-summary.ts`)
...
### Added
- **자막 오타(용어) 교정 파이프라인 + 어드민 교정 로그**: 요약 생성 시 gpt-5-nano 가 자막이 망가뜨린 고유명사·전문용어를 채널·제목 맥락에 맞게 교정하고, 표기형(한글/영어/하이브리드, 예 "키미 케이쓰리"→"키미 K3(Kimi K3)")을 정규화해 요약 본문에 반영한다. 교정 내역은 `term_corrections` 테이블에 적재되고, 어드민 신규 메뉴 **"오타 교정 로그"**에서 어떤 콘텐츠의 어떤 단어를 왜·어떻게 교정했는지 조회한다(콘텐츠 보기 모달·필터·검색). 관리자는 자동 교정을 수정하고 메모(추후 교정 품질 향상 학습데이터)를 남길 수 있으며, 수정된 교정은 재처리에 덮이지 않는다. 기존 요약 호출에 통합해 추가 LLM 비용은 없다. (`apps/web/lib/summary/*`, `term_corrections` + `record/get/save_term_correction`·`get_video_content` RPC, `apps/admin/**/corrections/*`)
...
### Added
- **어드민 용어사전 '콘텐츠' 열**: 각 용어가 도출된 원본 콘텐츠를 조회하는 링크를 정의 옆에 추가. 클릭하면 모달로 그 용어가 추출된 영상들의 요약 본문과 메타(제목·채널·게시일·영상 길이·썸네일·유튜브 링크)를 보여준다. (`apps/admin/**/glossary/*`, `get_glossary_sources` RPC)

## [0.17.0] - 2026-07-18

### Added
- **용어사전 다른 표기(Alias)**: 하나의 용어(대표 한글·영어)에 여러 다른 표기(키미3·키미 3·Kimi3·키미쓰리 등)를 등록하면, 요약 본문에 어떤 표기가 나와도 같은 용어 툴팁이 뜬다(툴팁 헤더는 대표명 표시). 어드민 용어사전 편집에 "다른 표기(Alias)" 입력 추가. (`get_video_glossary` alias 매칭, `lib/feed/render-terms.tsx`, `apps/admin/**/glossary/*`)
- **어드민 용어사전 CSV 다운로드/업로드**: 현재 필터·검색 결과를 CSV로 내려받아(대표 한글/영어·Alias·정의·메모 + 참고용 출처·상태·수정자·수정일) 엑셀에서 일괄 편집한 뒤 다시 업로드해 반영한다. 업로드는 id 기준으로 대표명·Alias·정의·메모만 갱신하며, 실제로 바뀐 행만 수정일이 갱신된다. (`apps/admin/**/glossary/*`, `import_glossary_csv` RPC, UTF-8 BOM)
...
### Changed
- **멤버십 플랜 변경 UX 개선**: 다운그레이드·해지 시 확인 모달에 이번 주기 유지 기간과 변경 후 한도(채널·다이제스트·AI 질의 감소, 채널 초과 시 일시정지 개수)를 함께 보여주고, 확인 버튼을 빨간색 "다운그레이드 예약"·"해지 예약"으로 표시한다. 변경 예약이 걸린 플랜은 카드에 "예약됨"으로 강조되고 그 자리에서 예약을 취소할 수 있다. (`components/membership/MembershipScreen.tsx`)

## [0.16.0] - 2026-07-18

### Changed
- **멤버십 플랜 변경 개방(Free·Small·Medium)**: PoC 기간 동안 잠겨 있던 플랜 변경을 열어 실제 업그레이드/다운그레이드를 사용할 수 있게 했다(Large는 PG 연동 전까지 "추후 오픈" 유지). 업그레이드는 즉시 반영, 다운그레이드·해지는 다음 결제 주기부터 적용(예약). 현재는 무료 체험(PoC·크레딧 결제 전) 기간이라 플랜을 바꿔도 과금은 0원이며, 다운그레이드 시 한도 초과 채널은 삭제가 아니라 일시정지된다(상위 플랜에서 직접 정지 해제). 얼리버드 배너도 현재 플랜명을 반영하도록 수정. (`components/membership/MembershipScreen.tsx`)

## [0.15.0] - 2026-07-18

### Added
- **멤버십 이용내역 탭**: 멤버십 메뉴 상단에 2개 탭(현재 멤버십 / 멤버십 이용내역)을 두고, 이용내역 탭에서 과거 구독·결제 이력을 카드형으로 조회한다. 카드마다 구독 기간·플랜·청구 금액·결제 금액·결제 수단·결제 상태·결제일을 표시하고, 월 중 플랜 업그레이드가 있으면 같은 카드 안에 함께 보여준다(이전→새 플랜·비례정산액). 결제는 크레딧 기반(PG는 향후)이라 결제 수단은 크레딧/PoC 무료로 표시하며 복합결제 UI 구조를 갖췄다. (`components/membership/{MembershipScreen,BillingHistoryCards}.tsx`, `lib/membership/history.ts`)
- **어드민 '멤버십 이력' 메뉴**: 구독자별 결제·업그레이드 이력을 상태 필터 + 이메일 검색 + 페이지네이션으로 조회(마스킹·KST). (`apps/admin/**/membership/*`, `get_membership_history` RPC)
...
### Added
- **용어 정의 인라인 툴팁**: 다이제스트 요약 본문에서 어려운 용어에 점선이 표시되고, 탭/클릭하면 그 자리에서 정의 툴팁이 뜬다(모달·AI 질의 없이 즉시). 표시 대상은 (a) 요약 시 함께 추출한 이 영상의 용어 + (b) 이미 전역 용어사전에 있고 본문에 등장하는 용어(하이브리드). 정의는 용어당 한 번만 생성해 재사용한다. 설정 > '용어 정의 툴팁'에서 끌 수 있다(기본 켜짐, 신규 영상부터 적용). (`components/feed/SummaryCard.tsx`, `TermTooltip.tsx`, `lib/feed/render-terms.tsx`, `lib/glossary/*`, `get_video_glossary` RPC)
- **어드민 '용어사전' 메뉴**: 전역 용어 정의를 출처(LLM/관리자) 필터 + 검색 + 페이지네이션으로 조회하고, 정의를 직접 수정할 수 있다(어드민 최초 쓰기 기능). 수정하면 출처가 '관리자'로 바뀌고 등록/수정자·일시 이력이 남으며, 이후 파이프라인이 그 정의를 덮어쓰지 않는다. (`apps/admin/**/glossary/*`, `get_glossary`·`get_glossary_history`·`edit_glossary_term` RPC)

### Changed
- **요약 오타 교정 확대**: 자막·음성 전사에서 생긴 명백한 띄어쓰기·조사·오탈자·동음이의 오인식까지 요약 반영 시 자연스럽게 교정한다(기존 고유명사 오인식 교정에 더해). 의미가 바뀌거나 불확실하면 원문을 유지한다(과교정 금지). 프롬프트 버전 갱신 → 신규 처리 영상부터 적용. (`lib/summary/summarize.ts`)
...
### Changed
- **홈 대시보드 다듬기**: '원본 영상 시간 누계' 라벨을 **'원본 영상 시간'**으로 축약. 두 카드(총 누적 다이제스트·압축 영상 시간 누계)의 **높이를 일치**(그리드 셀 stretch 를 Card 가 확실히 채우도록 flex 처리). (`components/home/HomeStatsGrid.tsx`)

## [0.11.2] - 2026-07-15

### Changed
- **홈 대시보드: '원본 영상 시간 누계'를 '총 누적 다이제스트' 카드 안으로 통합**: 별도 카드를 없애고 총 누적 다이제스트 카드의 하위 위계(약한 보조 수치)로 표시 → 대시보드가 2카드(총 누적 다이제스트 · 압축 영상 시간 누계)로 정리. (`components/home/HomeStatsGrid.tsx`)

## [0.11.1] - 2026-07-15

### Changed
- **홈 실적 대시보드를 누적 가치 중심으로 재구성**: '이번달 다이제스트'(히어로 문장과 중복)·'구독 중인 채널'(핵심 가치 요소 아님) 카드를 제거하고, 3카드를 **총 누적 다이제스트 → 원본 영상 시간 누계 → 압축 영상 시간 누계** 순으로 재편. 압축 영상 시간 누계 카드에 **아껴진 시간·압축률**을 약한 위계로 표시. 시간 값은 숫자 크게·단위 작게(bold 미사용). (`components/home/HomeStatsGrid.tsx`, `app/page.tsx`, `HomeDashboard.tsx`)

## [0.11.0] - 2026-07-15

### Changed
- **홈 상단 실적을 1×3 대시보드 그리드로 강조**: 기존 인라인 3칩(그동안 누적·이번달 누적 영상·구독 중인 채널)을 **대시보드 지표 카드**로 재편. 각 셀은 강조 실적 숫자(폰트 크게, bold 미사용 — 크기·색으로만 위계) + 약한 보조 수치(작게·톤다운) 구성. (`components/home/HomeStatsGrid.tsx`)
  - **총 누적 다이제스트** N개 + 원본 영상 시간·읽는 시간(누적).
  - **이번달 다이제스트** N개 + 원본 영상 시간·읽는 시간(이번달).
  - **구독 중인 채널** N개 + 일시정지 N개.
  - 누적 원본·읽는 시간은 기존 `get_month_value_stats` 를 재사용(스키마 변경 없음)해 집계. (`app/page.tsx`, `components/home/ValueHero.tsx`, `HomeDashboard.tsx`)

## [0.10.2] - 2026-07-15

### Removed
- **개발자 정보의 '리멤버 프로필' 링크 삭제**: 사이드패널 > 개발자 정보 하단 연락처에서 리멤버 프로필 링크를 제거(이메일·GitHub 링크는 유지). 미사용 아이콘·URL·문구도 함께 정리. (`app/developer/page.tsx`, `messages/ko.json`)
